...
#
# This code implements a modification of the first-order multifractal detrended fluctuation analysis (MF-DFA) method.
# It computes the Lipschitz-Holder multifractal singularity spectrum, as well as the minimum and maximum generalised
# Hurst exponents [2, 3]. For the core of the modification of the unifractal first-order DFA, please refer to [1].
#
# INPUT:
...
# normType        - any real greater than or equal to one, specifying the [fixed] p-norm
# isDFA           - a boolean value prescribing to use either the DFA-based algorithm (1) or
#                   the standard Hurst (R/S) analysis (0)
# isNormalised    - a boolean value prescribing to normalise the intermediate range-to-deviation (R / S) expression (1)
#                   or proceed computing with unnormalised value (0)
#
# OUTPUT:
# timeMeasure     - the time measure of the data's support at different scales
# dataMeasure     - the data measure at different scales computed for each value of the [variable] q-norm
# scales          - the scales at which the data measure is computed
# out             - the structure containing the MF-DFA statistics
# q               - the values of the q-norm
#
# Similarly to the unifractal function getHurstByUpscaling(), the time measure is computed merely for an alternative
...
def getMSSByUpscaling(dx, normType = np.inf, isDFA = 1, isNormalised = 1):
    ...
    return [timeMeasure, dataMeasure, scales, out, q]